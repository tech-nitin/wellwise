...
import { Compass, Layers, ShieldAlert, Bot, ArrowRight } from "lucide-react";
...
export function EngineerActions({ wellId }: EngineerActionsProps) {
  const actions = [
    {
  ...
  return (
    <div className="w-full bg-[#FAF8F5] border border-[#DDD2C0] rounded-xl px-3 py-2 shadow-2xs select-none">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
        <span className="text-[10px] font-mono font-bold text-[#8B877D] uppercase shrink-0">
          ENGINEER ACTIONS
        </span>

        {/* Compact Horizontal Buttons Strip */}
        <div className="flex flex-wrap items-center gap-1.5 w-full sm:w-auto justify-start sm:justify-end">
          {actions.map((act) => (
            <Link
              ...
              href={act.href}
              className={cn(
                "px-3 py-1.5 rounded-lg text-xs font-mono font-bold transition-colors cursor-pointer text-center",
                act.primary
