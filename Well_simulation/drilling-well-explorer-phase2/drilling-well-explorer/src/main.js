...
function animateStage(key) {
  ...
  const state = { x: camera.position.x, y: camera.position.y, z: camera.position.z, tx: controls.target.x, ty: controls.target.y, tz: controls.target.z, opacity: formationMats[0].opacity, drill: drillGroup.scale.y, casing: casings[0].material.opacity };
  activeTween = gsap.to(state, {
    ...
    opacity: data.formation, drill: data.drill, casing: data.casing,
    duration: 1.1, ease: 'power2.inOut',
    onUpdate: () => {
      camera.position.set(state.x, state.y, state.z);
      controls.target.set(state.tx, state.ty, state.tz);
      formationMats.forEach(m => { m.opacity = state.opacity; });
      drillGroup.scale.y = Math.max(0.03, state.drill);
      casings.forEach(c => { c.material.opacity = state.casing; });
    }
  });
...
function toggleSection() {
  sectioned = !sectioned;
  gsap.to(formations[4].scale, { z: sectioned ? 0.42 : 1, duration: 0.55, ease: 'power2.inOut' });
  gsap.to(formations[3].scale, { z: sectioned ? 0.66 : 1, duration: 0.55, ease: 'power2.inOut' });
}
...
document.querySelector('#timeline').addEventListener('input', e => animateStage(stageOrder[Math.round(Number(e.target.value) / 100 * (stageOrder.length - 1))]));
...
document.querySelector('#playBtn').addEventListener('click', () => {
  stopAuto(); let idx = stageOrder.indexOf(currentStage);
  autoTimer = setInterval(() => { idx += 1; if (idx >= stageOrder.length) { stopAuto(); idx = stageOrder.length - 1; } animateStage(stageOrder[idx]); }, 1900);
});
